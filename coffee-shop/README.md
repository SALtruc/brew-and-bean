...
The app works locally **without** AWS configured (images fall back to styled
placeholders, DynamoDB writes are skipped with a warning). When you add AWS
...
| Storage     | S3 (public bucket for product images)   |
...
Enable static public read access (for a learning project — in production, use
CloudFront with signed URLs). In the bucket's **Permissions** tab:

1. Untick **Block all public access**.
2. Add this bucket policy (replace bucket name):
...
Then upload images keeping the folder structure:
...
Set `NEXT_PUBLIC_S3_BASE_URL` in `.env.local`:
...
4. **IAM role:** create a role with these policies and attach to the instance:
   ...
   - `AmazonS3ReadOnlyAccess` (only if your app ever reads from S3 server-side)
...
- **Storage (S3)** holds product images. Because the bucket is public, the
  browser fetches images directly from S3 — the Next.js server never touches
  them, which is much cheaper and faster than proxying.
